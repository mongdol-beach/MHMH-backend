...
public interface TopicRepository extends JpaRepository<TopicEntity, Integer> {
    @Query(value = "SELECT * FROM topic TABLESAMPLE BERNOULLI(90) LIMIT :limit", nativeQuery = true)
    List<TopicEntity> findTopicByLimit(@Param("limit") int limit);

    @Query(value = "SELECT * FROM topic TABLESAMPLE BERNOULLI(90) WHERE situation_type=:situation LIMIT :limit", nativeQuery = true)
