...
public interface TopicRepository extends JpaRepository<TopicEntity, Integer> {
    @Query(value = "SELECT * FROM topic TABLESAMPLE BERNOULLI(5) LIMIT :limit", nativeQuery = true)
    List<TopicEntity> findTopicByLimit(@Param("limit") int limit);

    @Query(value = "SELECT * FROM topic TABLESAMPLE BERNOULLI(25) WHERE situation_type=:situation LIMIT :limit", nativeQuery = true)
